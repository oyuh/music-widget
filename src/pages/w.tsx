  ...
  <div style={{ textAlign: 'right', minWidth: 0 }}>
        ...
        {cfg.fields.progress && (
          ...
          <div style={{ height: "100%", width: `${percent}%`, background: computedAccent, transition: "width 120ms linear" }} />
            ...
            {cfg.layout.showArt && <img src={artSrc ? `/api/proxy-image?url=${encodeURIComponent(artSrc)}` : "/window.svg"} onError={(e)=>{ (e.currentTarget as HTMLImageElement).src = "/window.svg"; }} alt="" style={{ width: cfg.layout.artSize, height: cfg.layout.artSize, objectFit: "cover", borderRadius: 12, justifySelf: 'end' }} />}
          </>
        ) : (
          <>
            {cfg.layout.showArt && <img src={artSrc ? `/api/proxy-image?url=${encodeURIComponent(artSrc)}` : "/window.svg"} onError={(e)=>{ (e.currentTarget as HTMLImageElement).src = "/window.svg"; }} alt="" style={{ width: cfg.layout.artSize, height: cfg.layout.artSize, objectFit: "cover", borderRadius: 12, justifySelf: 'start' }} />}
