...
// Increased intervals to reduce shared Worker request consumption
const INTERVALS = {
  FAST: 3000,        // When track just changed or user is active
  NORMAL: 6000,      // When actively playing
...
export function useNowPlaying(options: {
    ...
    const fetchDurationAsync = async (tr: LfmTrack) => {
      try {
        const infoUrl = sessionKey
          ...
          : `/api/lastfm/trackInfo?artist=${encodeURIComponent(tr.artist["#text"])}&track=${encodeURIComponent(tr.name)}`;
  ...
  }, [username, sessionKey, getOptimalInterval, detectPauseState, estimateTrackPosition]);
