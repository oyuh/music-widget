<script lang="ts">
  ...
  const contentId = `collapsible-${componentId}`;
</script>

<section class="overflow-hidden rounded-md border bg-zinc-900/40 transition-colors {open ? 'border-border' : 'border-border/60'}">
  ...
  <div class="flex items-center">
    <button
      ...
      class="flex min-h-9 min-w-0 flex-1 items-center justify-between gap-2 px-2.5 py-1.5 text-left transition-colors hover:bg-muted/60 focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-primary {open ? 'bg-zinc-800/45' : ''}"
    >
      <span class="flex min-w-0 items-center gap-1.5 font-mono-ui text-xs font-medium tracking-wide uppercase {open ? 'text-foreground' : 'text-muted-foreground'}">
        {#if icon && ICONS[icon]}
      ...
      <span class="flex shrink-0 items-center gap-2">
        {#if badge}<span class="text-xs tabular-nums text-muted-foreground">{badge}</span>{/if}
        <svg
          class="h-3.5 w-3.5 shrink-0 text-muted-foreground transition-transform duration-150 {open ? 'rotate-90' : ''}"
          viewBox="0 0 12 12"
          fill="none"
          aria-hidden="true"
        >
  ...
  {#if open}
    <div id={contentId} class="flex flex-col gap-2.5 border-t border-border/50 px-2.5 py-2.5">
      {@render children?.()}
    </div>
  {/if}
</section>
