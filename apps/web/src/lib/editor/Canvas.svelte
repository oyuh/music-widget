<script lang="ts">
  ...
  // "Star" with no count rather than showing a bare zero.
  let stars = $state<number | null>(null);
  onMount(() => {
    void fetchGithubStars().then((n) => (stars = n));
  });
...
>
  ...
  <footer class="font-mono-ui relative z-20 flex shrink-0 items-center justify-between gap-3 px-4 py-2.5 text-[11px] leading-relaxed text-muted-foreground">
    ...
    <div class="flex shrink-0 items-center gap-1.5" data-support>
      ...
      <!-- Star: the repo link doubles as a nudge, with the live star count beside it. -->
      <a
        href={LINKS.github}
        target="_blank"
      ...
      >
        <svg viewBox="0 0 24 24" class="h-3.5 w-3.5" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
          <path d="m12 2.5 2.9 5.9 6.6.95-4.75 4.65 1.1 6.5-5.85-3.1-5.85 3.1 1.1-6.5L2.5 9.35l6.6-.95Z" />
        </svg>
        Star
        {#if starCount}
          <span class="-mr-0.5 ml-0.5 border-l border-border py-0.5 pl-1.5 tabular-nums">{starCount}</span>
        {/if}
      </a>
      <div class="relative">
        <button
